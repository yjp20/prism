...
export default class HttpOperationOptionsNegotiator implements IOperationOptionsNegotiator<IHttpOperation, IHttpOperationOptions, IHttpRequest,IHttpOperationOptionsNegotiationResult> {
    ...
    public negotiate(httpOperation: IHttpOperation, desiredOptions: IHttpOperationOptions, httpRequest: IHttpRequest): Promise<IHttpOperationOptionsNegotiationResult> {
        try {
            ...
            if (!validationResult.isValid) {
                ...
                httpOperationOptions = this.negotiateOptionsForValidRequest(httpOperation, desiredOptions, httpRequest);
            }
    ...
    private negotiateOptionsForValidRequest(httpOperation: IHttpOperation, desiredOptions: IHttpOperationOptions, httpRequest: IHttpRequest): IHttpOperationOptions {
        let { code } = desiredOptions;
        if (code) {
            return this.negotiateOptionsBySpecificCode(httpOperation, desiredOptions, httpRequest, code);
        }
        return this.negotiateOptionsForDefaultCode(httpOperation, desiredOptions, httpRequest);
    }

    private negotiateOptionsForDefaultCode(httpOperation: IHttpOperation, desiredOptions: IHttpOperationOptions, httpRequest: IHttpRequest): IHttpOperationOptions {
        const lowest2xxResponse = this.findLowest2xx(httpOperation.responses);
        if (lowest2xxResponse) {
            return this.negotiateOptionsBySpecificResponse(httpOperation, desiredOptions, httpRequest, lowest2xxResponse);
        }
        throw new Error('No 2** response defined, cannot mock');
    }

    private negotiateOptionsBySpecificResponse(httpOperation: IHttpOperation, desiredOptions: IHttpOperationOptions, httpRequest: IHttpRequest, response: IHttpResponse): IHttpOperationOptions {
        const { code } = response;
        let { _example: exampleKey } = httpRequest.query;
        let { mediaType, dynamic } = desiredOptions;
    ...
    private negotiateOptionsBySpecificCode(httpOperation: IHttpOperation, desiredOptions: IHttpOperationOptions, httpRequest: IHttpRequest, code: string): IHttpOperationOptions {
        let mediaType;
        let exampleKey;
        let dynamic;
        ...
        if (responseByForcedStatusCode) {
            try {
                // try to negotiate
                return this.negotiateOptionsBySpecificResponse(httpOperation, desiredOptions, httpRequest, responseByForcedStatusCode);
            } catch (error) {
